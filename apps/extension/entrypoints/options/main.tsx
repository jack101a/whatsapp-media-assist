...
const QUICK_START_TEMPLATES: Array<{ name: string; tag: string; description: string; steps: PipelineStep[] }> = [
  {
    name: 'Photo Upload',
    tag: 'PHOTO',
    description: 'Rotate if needed → Crop to 1:1 → Resize 160×200 → JPEG → Under 50 KB',
    steps: [
  ...
  },
  {
    name: 'Signature',
    tag: 'SIGN',
    description: 'Crop free → Resize 256×64 → JPEG → Under 20 KB',
    steps: [
  ...
  },
  {
    name: 'Passport Photo',
    tag: 'PASS',
    description: 'Crop free → Resize 413×531 → JPEG → Under 100 KB',
    steps: [
      { id: '', type: 'crop', mode: 'ask', ratio: 'free' },
...
function stepSummary(step: PipelineStep): string {
  ...
  if (step.type === 'rotate') return `${step.degrees > 0 ? step.degrees + '° right' : Math.abs(step.degrees) + '° left'}`;
  if (step.type === 'resize') return [step.width, step.height].filter(Boolean).join('×') || 'Custom';
...
function StepEditor({ step, onChange, onDelete, onDragStart, onDragOver, onDrop }: {
  ...
  return <article className="step-card" draggable onDragStart={onDragStart} onDragOver={onDragOver} onDrop={onDrop}>
    <div className="step-head">
      <div className="step-drag-handle" title="Drag to reorder">⋮⋮</div>
      <strong>{stepLabel[step.type]}</strong>
      <button className="delete" onClick={onDelete}><Icon name="trash" /></button>
    </div>
  ...
  return <main className="page">
    ...
    <div className="shell">
      ...
      <section className="content">
        ...
        {tab === 'pipelines' && <>
          <Title title="Pipelines" action={<button className="btn primary" disabled={!premium} onClick={() => setDraft(blankPipeline())}><Icon name="plus" />New pipeline</button>} />
          ...
          {premium && !draft && (
            <>
              <div className="title" style={{ marginTop: 0 }}><h2 style={{ fontSize: '14px', color: 'var(--text-muted, #64748b)', fontWeight: 500 }}>Quick-start templates</h2></div>
              <div className="quickstart-grid">
                {QUICK_START_TEMPLATES.map((template) => (
                  <button key={template.name} className="quickstart-card" onClick={() => setDraft(makeQuickStartPipeline(template))}>
                    ...
                    <em>Click to customize &rarr;</em>
                  </button>
                ))}
              </div>
            </>
          )}
          {draft && <Card title={form.profiles.some((item) => item.id === draft.id) ? 'Edit pipeline' : 'New pipeline'} action={<button className="icon-btn" onClick={() => setDraft(null)}><Icon name="close" /></button>}>
