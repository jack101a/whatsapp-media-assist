  ...
  return <main className="page">
    ...
    <div className="shell">
      ...
      <section className="content">
        ...
        {tab === 'crop' && <><Title title="Crop" /><Card title="Default crop preset"><div className="grid"><Field label="Preset"><select className="select" value={form.defaultCropPresetId ?? ''} disabled={!premium || !cropTemplates.length} onChange={(e) => void applyDefaultPreset(e.target.value, 'defaultCropPresetId', cropTemplates, IMAGE_TEMPLATE_KEYS)}><option value="">Manual crop settings</option>{cropTemplates.map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></Field><Field label="Crop ratio"><select className="select" value={form.defaultCropRatio} onChange={(e) => patch('defaultCropRatio', e.target.value as CropRatio)}><option value="free">Free</option><option value="original">Original</option><option value="1:1">1:1</option><option value="3:4">3:4</option><option value="4:3">4:3</option><option value="16:9">16:9</option></select></Field></div></Card><ServerPresetList title="Available crop presets" templates={cropTemplates} disabled={!premium} onApply={(template) => void applyServerTemplate(template)} /></>}
        {tab === 'resize' && <><Title title="Resize" /><Card title="Default resize preset"><div className="grid"><Field label="Preset"><select className="select" value={form.defaultResizePresetId ?? ''} disabled={!premium || !resizeTemplates.length} onChange={(e) => void applyDefaultPreset(e.target.value, 'defaultResizePresetId', resizeTemplates, IMAGE_TEMPLATE_KEYS)}><option value="">Manual resize settings</option>{resizeTemplates.map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></Field><Field label="Width"><NumberField value={form.defaultWidth} onChange={(value) => patch('defaultWidth', value)} /></Field><Field label="Height"><NumberField value={form.defaultHeight} onChange={(value) => patch('defaultHeight', value)} /></Field><Field label="Fit"><select className="select" value={form.defaultResizeFit} onChange={(e) => patch('defaultResizeFit', e.target.value as AppSettings['defaultResizeFit'])}><option value="contain">Contain</option><option value="cover">Cover</option><option value="stretch">Stretch</option></select></Field><Toggle checked={form.allowUpscale} onChange={(value) => patch('allowUpscale', value)} label="Allow enlargement" /></div></Card><ServerPresetList title="Available resize presets" templates={resizeTemplates} disabled={!premium} onApply={(template) => void applyServerTemplate(template)} /></>}
        {tab === 'compress' && <><Title title="Compress" /><Card title="Default compression preset"><div className="grid"><Field label="Preset"><select className="select" value={form.defaultCompressPresetId ?? ''} disabled={!premium || !compressTemplates.length} onChange={(e) => void applyDefaultPreset(e.target.value, 'defaultCompressPresetId', compressTemplates, IMAGE_TEMPLATE_KEYS)}><option value="">Manual compression settings</option>{compressTemplates.map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></Field><Field label="Format"><select className="select" value={form.defaultFormat} onChange={(e) => patch('defaultFormat', e.target.value as ImageFormat)}><option value="jpeg">JPEG</option><option value="png">PNG</option><option value="webp">WebP</option></select></Field><Field label="Minimum KB"><NumberField value={form.defaultMinKB} onChange={(value) => patch('defaultMinKB', value)} /></Field><Field label="Maximum KB"><NumberField value={form.defaultMaxKB} onChange={(value) => patch('defaultMaxKB', value)} /></Field><Field label="Quality %"><input className="input" type="number" min="35" max="100" value={form.defaultQuality} onChange={(e) => patch('defaultQuality', Number(e.target.value))} /></Field><Field label="Minimum quality"><input className="input" type="number" min="20" max="90" value={form.minimumQuality} onChange={(e) => patch('minimumQuality', Number(e.target.value))} /></Field><Toggle checked={form.allowDimensionReduction} onChange={(value) => patch('allowDimensionReduction', value)} label="Reduce dimensions if needed" /></div></Card><ServerPresetList title="Available compression presets" templates={compressTemplates} disabled={!premium} onApply={(template) => void applyServerTemplate(template)} /></>}
