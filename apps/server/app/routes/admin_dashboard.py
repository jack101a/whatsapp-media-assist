...
def get_system_settings(db: Session = Depends(get_db), settings_env: Settings = Depends(get_settings)) -> dict:
    ...
    # Inject Razorpay environment secrets (read-only)
    settings_dict["razorpay_webhook_secret"] = settings_env.razorpay_webhook_secret
    base = str(settings_env.api_base_url).rstrip("/") if settings_env.api_base_url else "https://<your-domain>"
