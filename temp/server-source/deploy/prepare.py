...
if SERVER_ENV.exists() or DEPLOY_ENV.exists():
    raise SystemExit('server/.env or deploy/.env already exists. Remove them only if you intentionally want to regenerate secrets.')
...
DEPLOY_ENV.write_text(f'POSTGRES_PASSWORD={db_password}\n', encoding='utf-8')
SERVER_ENV.chmod(0o600)
DEPLOY_ENV.chmod(0o600)
print('Created server/.env and deploy/.env with unique local secrets.')
