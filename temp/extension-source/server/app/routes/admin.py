...
from ..models import Plan, Subscription, User, utcnow
...
def grant(email: str, days: int = 365, db: Session = Depends(get_db)) -> MessageResponse:
    ...
    db.add(Subscription(user_id=user.id, plan_id=plan.id if plan else None, status='active', starts_at=start, expires_at=start + timedelta(days=days), source='admin', source_payment_id=f'admin-{user.id}-{int(utcnow().timestamp())}', currency='INR', amount_minor=0))
    db.commit()
