...
function newStep(type: PipelineStep['type']): PipelineStep {
  ...
  switch (type) {
    ...
    case 'compress': return { id, type, minKB: 100, maxKB: 200 };
...
function Options() {
  ...
  const refreshBilling = async () => {
    ...
    try {
      ...
      setBilling(status);
      if (status.signedIn) setAccount(await billingRequest<AccountData>({ type: 'billing:get-account' }));
